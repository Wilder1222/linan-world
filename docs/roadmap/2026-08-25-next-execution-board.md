...
## SG-02 关系、活动、幽默与可替换性独立审读结果（2026-08-26）

- 新增 `scripts/audit_season_gate_relationship_life_humor.py` 与 `tests/test_season_gate_relationship_life_humor.py`，报告为 `REVIEWED-SEASON-PASS`。
- 17 条关系均保留 8 个阶段快照（136 条证据），36 集关系选择均有选择、状态变化和人物引用；17 条关系全部被季级输入使用。
- 36 个宋代生活活动绑定覆盖 36 集；每项都有历史锚点、关系或线索变化、选择、状态转移和连续性代价；重复活动的状态转移不相同。
- 36 个角色化幽默绑定覆盖 36 集；笑点有说话者意图、听者反应、情绪回收、禁用目标和时代转译；高风险阶段具备 tonal safety，未发现现代网络词。
