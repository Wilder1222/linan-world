...
本包完成的是已锁定 Season Gate 所要求的沈蘅第一季角色视觉资产；不是 Episode Gate 后的逐场成片、对白或镜头交付。Episode Gate 当前为 `OPEN`，未来锁定时应按新输入哈希复审，但这不构成当前季级资产缺口。

## 要求与证据

| 要求 | 权威证据 | 结论 |
|---|---|---|
| 使用 `raw/` 三张母版保持身份 | `season-1-coverage-plan.json` 中三张 SHA-256；`asset-audit.json` 的 `identityReferenceHashParity` | 通过 |
| 按项目设定和第一季剧本生成 | `qa/gates/season-gate.json` 为 `LOCKED`；人物源、Gate、输入 manifest 和六份季级矩阵哈希全部匹配 | 通过 |
...
| 工程兼容 | 60 项单元测试通过；`validate_project.py --scope manifest --strict` 通过 | 通过 |
