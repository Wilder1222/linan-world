...
- 身份锁定、角度、三视图、服装结构和道具优先使用 `--raw`；主角定妆、叙事剧照、春台和节庆可不使用 Raw，以保留 V8.2 的构图与华美表达。
- 使用中低 `--s` 与低 `--c` 锁定连续性；角色主视觉、叙事剧照和春台/节庆按资产线提高 `--s`，但不能以高风格化替代设计判断。
- `--no` 默认省略。只有质量复核证实单一故障持续出现时才附加一到两个安全的单词级目标，例如 `--no text, watermark`；不对脸、皮肤、身体、衣物或物件使用负向参数。
